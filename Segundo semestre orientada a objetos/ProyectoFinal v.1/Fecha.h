...
int main()
{
	...
	pos = names[2].find(word);
	cout << pos;

	/*pos = names[0].find(word);
	if (pos >= 0);
	else
	pos = 0;
	int lenght = word.length();

	cout << names[0].substr(pos, lenght);*/

	cout << endl << binarySearch(0, j - 1, names, word);

	//cout << endl << binarySearch(0, j - 1, names, "a");

	/*std::string str = names[0] "We think in generalities, but we live in details." ;

	std::string str2 = names[0].substr(3, 5);     // "think"

	std::size_t pos = names[0].find("ark");      // position of "live" in str

	std::string str3 = names[0].substr(pos);     // get from "live" to the end

	std::cout << str2 << endl << pos << ' ' << str3 << '\n';*/
