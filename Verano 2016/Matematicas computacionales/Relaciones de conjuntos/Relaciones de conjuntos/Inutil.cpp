...
void createWords(string inductive[], int induc, string solucion[], int* sol, char variable[], int* var, string basis[], int* basisElems);

int main()
{
	string::size_type size; //Size of the substring with the specified data type

	const int basisSize = 5;
	const int inductiveSize = 15;
	string timeS;
	int numliterals = 0;
	int times;
	int basisElems = -1;
	...
	createWords(inductiveStep, inductiveElems, solution, &sol, variables, &variableElems, basis, &basisElems);
	...
	for (int i = 0; i < induc; i++)
	{
		*var = 0;
		...
		for (int j = 0; j < *var; j++)
		{
			...
			{
				solucion[s] = remplazar(solucion[s], variable[j], basis[variableB]);
				//cout << solucion[s] << " ";
				s++;
			}
			s = *sol;
			...
			for (int k = 0; k < *basisElems; k++)
			{
				for (int l = 0; l < *var; l++)
				{
					solucion[s] = remplazar(solucion[s], variable[l], basis[k]);
					(k % 2 == 0) ? s++ : s--;
				}
