...
void GraphicAVLTree<T>::printGraphical()
{
	...
	while (window.isOpen())
	{
       ...
       // recursivePrintGraphical(this->root, 280, 34, window, font);
		recursivePrintGraphical(this->root, 279, 222, window, font);
		window.display();
	}
}
...
int GraphicAVLTree<T>::recursivePrintGraphical(GraphicNode<T> * _root, int x, int y, sf::RenderWindow & window, const sf::Font & font)
{
    if(_root != nullptr)
    {
        // Draw left branch
        if(_root->getLeft())
			//recursivePrintGraphical(static_cast<GraphicNode<T>*>(_root->getLeft()), x - (DISTANCE * cos(0.325) * pow(1.5, _root->getHeight() - 2)), y - (DISTANCE * sin(0.325) * pow(1.3, _root->getHeight() - 2)), window, font);
        // Draw the node
        _root->configure(font);
        _root->setPosition(sf::Vector2f(x, y));
        //std::cout << "Drawing node: " << _root->getData() << " at position: " << x << ", " << y << std::endl;
        _root->draw(window);
        // Draw right branch
        if(_root->getRight())
			//recursivePrintGraphical(static_cast<GraphicNode<T>*>(_root->getRight()), x + (DISTANCE * cos(0.325) * pow(1.5, _root->getHeight() - 2)), y - (DISTANCE * sin(0.325) * pow(1.3, _root->getHeight() - 2)), window, font);
		// Draw links betwen nodes
        if(_root->getLeft())
        {
			//_root->drawLeftLine(window);
        }

        if(_root->getRight())
        {
			//_root->drawRightLine(window);
        }

		return x + (DISTANCE * cos(0.325) * pow(2, _root->getHeight() - 1));
    }
    return 0;
}
