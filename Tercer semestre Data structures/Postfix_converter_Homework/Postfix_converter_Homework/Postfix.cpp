...
float Postfix::evaluate()
{
	...
	for (int i = 0; i < postfix_expression.length(); i++)
	{
		...
		{
			stack.push(postfix_expression[i]-'0');
		}
		...
		else
		{
			...
			{
				// Test whether the current token is higher or lower precedence
				result = compute(stack.pop()->getData(), stack.pop()->getData(), postfix_expression[i]);
				stack.push(result);
			}
		}
	}
...
float Postfix::compute(int num1, int num2, char sign)
{
	float result;
