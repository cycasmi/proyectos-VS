...
//*************************************************************************
//apple section altered. Look for marker
	...
	GLfloat lightpos[4])
{
	...
	shadowMat[1][0] = 0.0 - lightpos[X] * groundplane[Y];
	shadowMat[2][0] = 0.0 - lightpos[X] * groundplane[Z];
	shadowMat[3][0] = 0.0 - lightpos[X] * groundplane[W];

	shadowMat[X][1] = 0.0 - lightpos[Y] * groundplane[X];
	shadowMat[1][1] = dot - lightpos[Y] * groundplane[Y];
	shadowMat[2][1] = 0.0 - lightpos[Y] * groundplane[Z];
	shadowMat[3][1] = 0.0 - lightpos[Y] * groundplane[W];

	shadowMat[X][2] = 0.0 - lightpos[Z] * groundplane[X];
	shadowMat[1][2] = 0.0 - lightpos[Z] * groundplane[Y];
	shadowMat[2][2] = dot - lightpos[Z] * groundplane[Z];
	shadowMat[3][2] = 0.0 - lightpos[Z] * groundplane[W];

	shadowMat[X][3] = 0.0 - lightpos[W] * groundplane[X];
	shadowMat[1][3] = 0.0 - lightpos[W] * groundplane[Y];
	shadowMat[2][3] = 0.0 - lightpos[W] * groundplane[Z];
	...
	GLdouble thickness, GLuint side, GLuint edge, GLuint whole)
{
	...
	if (tobj == NULL)
	{
		tobj = gluNewTess();  // create and initialize a GLU 2polygon tesselation object 
		////********UNCOMMENT FOLLOWING LINES FOR MAC************
		//gluTessCallback(tobj, GLU_TESS_BEGIN, (_GLUfuncptr)glBegin);
		//gluTessCallback(tobj, GLU_TESS_VERTEX, (_GLUfuncptr)glVertex2fv);  // semi-tricky 
...
int main(int argc, char **argv)
{
	...
	glMatrixMode(GL_PROJECTION); //field of view in degree, aspect ratio and Znear, then Zfar
	gluPerspective(40.0, 1.0, 20.0, 100.0);
	glMatrixMode(GL_MODELVIEW); //The eye is at the first, center is the second value and up is the positive Y direction
	gluLookAt(0.0, 8.0, 60.0, 0.0, 8.0, 0.0, 0.0, 1.0, 0.);
